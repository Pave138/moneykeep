...
async def create_expense(
    obj_in: ExpenseCreate, session: SessionDep, user: UserDep
):
...
async def get_expense_by_user(user: UserDep, session: SessionDep):
...
async def update_expense(
    expense_id: int, obj_in: ExpenseUpdate, user: UserDep, session: SessionDep
):
...
async def delete_expense(
    expense_id: int, user: UserDep, session: SessionDep
):
    expense = await check_expense_exists(expense_id, session)
